...
pub fn client_typescript(runtime: &str) -> String {
    ...
    writeln!(o, "import {{ Client, type ClientSyncState, type Connection, type ConnectionOptions, type QuerySpec, type RecordValue, type ServerOptions }} from \"{runtime}\";").unwrap();
    writeln!(o, "export {{ type ServerOptions, type Connection, type ConnectionOptions }} from \"{runtime}\";").unwrap();
    o.push_str("import { schema, liveModels, GeneratedTransaction, Mutate, type LiveModels } from \"./generated.ts\";\n");
    ...
    o.push_str(" private constructor(client: Client, connection: Connection | undefined) { this.client = client; this.connection = connection; this.models = liveModels(client); this.mutate = new Mutate(client); this.channels = new Channels(client); }\n");
    o.push_str(" /** Opens the local database at `path`. With a server, the connection starts immediately and retries on its own. */\n");
    o.push_str(" static async open(options: { path: string; server?: ServerOptions; connection?: ConnectionOptions; migration?: { defaults?: RecordValue; replayPull?: boolean } }): Promise<GeneratedClient> {\n");
