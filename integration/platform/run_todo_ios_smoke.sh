...
bundle_id="dev.ahead.Todo"
app_bundle="${AHEAD_TODO_APP_BUNDLE:-$app_dir/ios/build/Build/Products/Release-iphonesimulator/AheadTodo.app}"
run_dir="$(mktemp -d "${TMPDIR:-/tmp}/ahead-todo-smoke.XXXXXX")"
backend_pid=""
created_devices=()
alice="${1:-}"
bob="${2:-}"
...
trap cleanup EXIT
if [[ ! -d "$app_bundle" ]]; then echo "Build the Release simulator app first (npm run ios:build in examples/todo/mobile); set AHEAD_TODO_APP_BUNDLE to its .app path." >&2; exit 1; fi
if [[ -z "$alice" && -z "$bob" ]]; then
  # Default to the newest installed iOS runtime and the first iPhone device type it supports.
  runtime="${AHEAD_TODO_SIM_RUNTIME:-$(xcrun simctl list runtimes -j | python3 -c 'import json,sys;r=[x for x in json.load(sys.stdin)["runtimes"] if x["platform"]=="iOS" and x["isAvailable"]];r.sort(key=lambda x:[int(p) for p in x["version"].split(".")]);print(r[-1]["identifier"])')}"
  kind="${AHEAD_TODO_SIM_DEVICE:-$(xcrun simctl list devicetypes -j | python3 -c 'import json,sys;print(next(x["identifier"] for x in json.load(sys.stdin)["devicetypes"] if x["identifier"].startswith("com.apple.CoreSimulator.SimDeviceType.iPhone-")))')}"
  alice="$(xcrun simctl create "Ahead Todo Alice $$" "$kind" "$runtime")";created_devices+=("$alice")
  bob="$(xcrun simctl create "Ahead Todo Bob $$" "$kind" "$runtime")";created_devices+=("$bob")
fi
...
launch_phase() {
  ...
  python3 - "$data" "$user" "$phase" "$AHEAD_SMOKE_PORTS" <<'PYCONF'
import json,sys
