...
- A **handler** receives `{input, tx, userId, changes, publish}`: the decoded input (one value per slot), the transaction, the user id, the mutation's change set and a way to publish. `changes` starts as the records the uploaded operations target; `changes.add(record)` reports a record the handler changed beyond them. `publish({channel})` publishes the mutation's final change set to a channel; `publish({channel, records})` publishes exactly those records (`[]` nothing). Its return value is ignored. Throwing `MutationRejected`, or an error `translateRejection` maps to a code, rejects that one mutation; any other error rejects that same mutation with `handler.failed` and is reported to `onError` — the rest of the batch still runs and commits ([#95](https://github.com/zanminwang/ahead/issues/95)). What the framework does with the change set after the handler returns is [Push](engine/push.md).
- A **loader** serves one retained model read contract (a model name and version) and receives `{ids, tx, userId}`: identities, the transaction and the user id, never a channel. It returns one row or `null` per identity, in order, shaped as that version's records. `null` means "not visible or deleted" and is delivered as a delete; a missing entry or `undefined` is a defect. Throwing `MutationRejected` (or an error `translateRejection` maps) refuses the read: inside a push that rejects the mutation being read back; in a pull it fails the page with `loader.refused` ([Server / Engine / Pull §9](engine/pull.md#9-architecture-decisions)). Any other error thrown during a push's readback rejects that mutation with `loader.failed` and is reported to `onError`; the same error during a pull still fails the page.
...
**Collecting changes and publications.** While a handler runs, `changes.add` and `publish` only record intent; `publish` validates its channel and records at the call. When the handler returns, the host answers `handle` with `{changes, publications}`: the final change set and the publications in call order, each with its explicit records or none. The engine then allocates the stamps, reads the change set back through `load` and carries out the publications ([Push](engine/push.md), [Publish](engine/publish.md)). A handler that publishes nothing is a normal success: its records are still stamped and read back and the receipt still carries them.

**Rejection versus failure.** `handle` and `load` each answer one of three things: a rejection (a value: `{rejection}`), a failure (also a value: `{error}`, what the TypeScript host answers for a thrown handler or loader error that is neither `MutationRejected` nor a `translateRejection` code), or a thrown error that crosses the host boundary itself. A rejection and a failure both roll back only that mutation's savepoint and continue with the next: `handler.failed` and `loader.failed` are the engine's codes for a handler's or a loader's `Failed` answer, and the host has already passed the underlying error to `onError`. Only a thrown error — a persistence fault, or a host too old to answer `Failed` — propagates out of the callback and fails the outer transaction, so the whole batch, its publications and the client row roll back together; this is infrastructure, not a business outcome (guarantees P6, P7). Loaders follow the same three-way rule, with one pull-side difference: a defective result aborts the pull rather than advancing the client's cursor past bad data, a refusal answered as a value (`{rejection}`) is the mutation's rejection in a push or `loader.refused` in a pull, and a `Failed` answer is `loader.failed` in a push but still fails the page in a pull.
...
| `handle` | `name`, `version`, `arguments`, `owner`, `ordinal` | `Handled::Settled { changes: Vec<RecordRef>, publications: Vec<PublicationIntent { channel, records: Option<Vec<RecordRef>> }> }`, `Handled::Rejected { rejection }`, or `Handled::Failed { error: String }` |
...
| `loader.refused` | `load` answering `{rejection}` while serving a page; in a push the same answer is the mutation's rejection |
...
- A `handle` answer carrying `changes` or `publications` beside `rejection`, or only one of `changes` and `publications`, is refused with `handler.invalid` ("invalid handler settlement"), as is a change or publication record without a model or an object identity. A `load` answer that is neither a row list, `{rejection}` with a valid code, nor `{error}` is `loader.invalid`.
...
- **The host answers `handle` with the change set and the publications, a rejection, or a failure, never more than one of the three; `load` answers rows, a refusal, or a failure; every one of the twelve operations round-trips through the fixture on both sides.** Evidence: [server/tests/host_contract.rs](../../../../crates/server/tests/host_contract.rs) `the_operation_list_matches_the_request_enum`, `the_fixture_covers_every_operation_exactly_once`, `every_fixture_request_and_response_round_trips`, `a_handle_response_carries_changes_and_publications_or_a_rejection_and_never_both`, `a_load_response_is_rows_or_a_refusal_code`, `a_response_of_the_wrong_type_is_refused_per_operation`; [host-contract.test.mjs](../../../../integration/persistence/server/host-contract.test.mjs) `the fixture and the TypeScript union cover the same operations`, `every fixture request replays through the TypeScript host to the fixture answer`, `the same handle request settles as the fixture rejection when the handler refuses`.
- **A handler that publishes nothing still succeeds with readback records; `publish({channel})` covers additions made after the call; explicit records publish only those; `publish` validates before the mutation settles; a handler may keep writing after `publish`.** Evidence: [runtime.test.mjs](../../../../integration/persistence/server/runtime.test.mjs) `a handler that publishes nothing still returns readback records and touches no channel`, `publish({channel}) publishes the final change set, an addition made after the call included`, `explicit records publish only those, never join the change set, and are initialised at stamp 1 once`, `publish validates channel and records before the mutation settles; the invalid call rejects only that mutation as a failure`, `a handler may keep using the transaction after publish; the publication goes out with the commit`, `slot arguments are tagged so changes.add and publish accept them directly`.
...
- **Loader defects abort the pull; `null` is a delete, `undefined` is a defect; loaders receive no channel.** Evidence: `loader defects abort pull instead of silently advancing its cursor`, `undefined loader entries remain defects and never become tombstones`, `loaders receive no channel`.
...
Executed 2026-09-15: `cargo test -p ahead-server --locked`, `cargo test -p ahead-sim --locked`, `bash integration/persistence/server/run.sh` (`runtime.test.mjs` and `host-contract.test.mjs`, 66 passed).
...
**The contract grew with readback ([#55](https://github.com/zanminwang/ahead/issues/55)) and with failure isolation ([#95](https://github.com/zanminwang/ahead/issues/95)).** `load` gained `version` for model read contracts ([#91](https://github.com/zanminwang/ahead/issues/91)) and lost `channel`; `handle` answers a change set and publications instead of a channel; `advanceStamp` and `ensureStamp` are new and `publish` takes the stamp instead of allocating one; `load` may answer a refusal. `#95` (mutation half) added the third `Failed { error }` answer to both `handle` and `load`, so the TypeScript host can report a thrown application error as data instead of letting it abort the delivery; a host that still throws is treated as infrastructure, as before. Twelve operations in all, each with one example in the fixture. Loader errors on pull pages are the read half of `#95` and remain open.

**Accepted limitation.** A loader must return exactly its version's contract fields: identity fields may be present, an absent nullable field reads as `null`, an absent non-nullable field or any extra property fails normalization and aborts the pull with a 500. Evidence: `normalize_state` over the retained contract in `process_pull`; the loader-defect test above and `a pull reaches the loader of the declared model version and normalizes rows with that contract`. Documented for authors under [Loaders](../../../../website/docs/backend/api.md#loaders).
