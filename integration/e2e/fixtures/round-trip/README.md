...
`bash integration/e2e/run.sh` regenerates and installs it. `run.sh` here serves the backend alone for manual use of `client.mts` (`AHEAD_URL`, `AHEAD_DATABASE`, `edit TEXT`, `offline`, `online`, `status`, `quit`).
