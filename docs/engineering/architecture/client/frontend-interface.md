...
| Sync | `freeze`, `acknowledge` (returns an `ApplyReport` for the receipt's authority; [Settlement](engine/settlement.md)), `apply_page`, `receive_downlink`, `downlink_request`, plus the `SyncCycle` and `ConnectionDriver` state machines |
| State and control | `pending_count`, `cursor`, `subscriptions`, `subscription_generation` (how many subscribes and unsubscribes committed since open; the [live session](connection/controller/live-session.md) restarts when it changes), `rejections`, `record_status`, `pending_tasks`, `next_task`, `outcome`, `set_readiness`, `drop_mutation`, `dismiss_rejection`, `schema_state` (`rebuilt`, `pending`, `last_rebuild`) |
...
Executed 2026-09-16: `cargo test -p ahead-sqlite -p ahead-binding --locked` passed with the tests above.
